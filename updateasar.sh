...
# Usage: ./updateasar.sh <path_to_new_windows_resources> <path_to_linux_resources>
# Example: ./updateasar.sh ~/Downloads/myDiabby-Uploader-2.42.0-win/resources ~/MyDiabby-Uploader-Linux-Work/files/resources
...
WORK_DIR="$(mktemp -d)"
NEW_WIN_RESOURCES="$1"
TARGET_LINUX_RESOURCES="$2"

# Cleanup on exit
trap 'rm -rf "$WORK_DIR"' EXIT

# Check arguments
if [ -z "$NEW_WIN_RESOURCES" ] || [ -z "$TARGET_LINUX_RESOURCES" ]; then
  echo "Usage: $0 <path_to_new_windows_resources> <path_to_linux_resources>"
  echo "Example: $0 ~/Downloads/myDiabby-Uploader-2.42.0-win/resources ~/MyDiabby-Uploader-Linux-Work/files/resources"
  exit 1
fi
...
if [ ! -d "$TARGET_LINUX_RESOURCES" ]; then
  echo "Error: target directory not found at $TARGET_LINUX_RESOURCES"
  exit 1
fi
...
echo "==> Copying updated asar to $TARGET_LINUX_RESOURCES..."
cp "$WORK_DIR/app.asar.final" "$TARGET_LINUX_RESOURCES/app.asar"

echo "==> Done! Updated app.asar has been copied to $TARGET_LINUX_RESOURCES"
